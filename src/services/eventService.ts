...
export const fetchEvents = async (): Promise<EventData[]> => {
  try {
    ...
    let data: EventData[]
    try {
      const apiResponse: ApiResponse<EventData[] | EventData> = await response.json()
      ...
      // Check if data field is empty or null
      if (!responseData) {
        return []
      }
      ...
      // Handle case where data might be an array directly
      if (Array.isArray(responseData)) {
        data = responseData.map((event: any) => ({
          ...
          createdAt: event.createdAt || event.created_at || undefined,
          ...event,
        }))
      } else if (typeof responseData === 'object') {
        // Handle nested data structure
        if (responseData.data && Array.isArray(responseData.data)) {
          data = responseData.data.map((event: any) => ({
            uuid: String(event.uuid || ''),
            eventName: String(event.eventName || ''),
            startDate: event.startDate || undefined,
            endDate: event.endDate || undefined,
          ...
          data = [{
            ...
            registrations: responseData.registrations || 0,
            createdBy: responseData.createdBy || responseData.created_by || undefined,
            createdAt: responseData.createdAt || responseData.created_at || undefined,
            ...responseData,
          }]
        }
      } else {
      ...
      // Validate that we have valid event data
      data = data.filter(event => event.uuid && event.eventName)
    } catch (parseError) {
      // Check if it's our custom validation error - re-throw it
      if (parseError instanceof Error && parseError.message.includes('Invalid response format')) {
        showToast.error(parseError.message)
